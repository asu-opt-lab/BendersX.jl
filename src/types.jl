...
Built-in environments currently operate on JuMP-backed masters through the public [`master_model`](@ref), [`linking_variables`](@ref), and [`auxiliary_variables`](@ref) interface. Model-based oracles additionally use [`copy_linking_variables!`](@ref), while sequential environments use [`evaluate_primal_objective`](@ref) and [`add_cuts!`](@ref).

Specializing [`update_master_model!`](@ref) changes the problem formulation while retaining the provided [`Master`](@ref) implementation. Defining a new `AbstractMaster` subtype instead changes the storage or behavior of the master itself.
