...
A master maintains the evolving master problem, provides the linking and auxiliary variables used by the Benders algorithm, and incorporates cuts generated by oracles.

`AbstractMaster` implementations expose the master problem through the [`master_model`](@ref), [`linking_variables`](@ref), and [`auxiliary_variables`](@ref) interfaces. Additional operations are provided through [`copy_linking_variable_tuple!`](@ref), [`evaluate_objective`](@ref), and [`add_cuts!`](@ref).

Specializing [`update_master_model!`](@ref) defines a problem-specific master formulation while retaining the provided [`Master`](@ref) implementation. Defining a new `AbstractMaster` subtype instead allows the master representation or behavior to be changed.
